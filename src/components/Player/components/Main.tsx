...
const Player: FC<PlayerProps> = ({
  ...
  const [isMuted, setIsMuted] = useState(Boolean(Number(localStorage.getItem('tuby-muted'))));
  ...
  const handleSeeking = (offsetX: number) => {
    ...
    const newTime =
      (Math.abs(offset) === Infinity || isNaN(offset) ? 0 : offset) * playerRef.current.duration;
  ...
  useEffect(() => {
    const keyHandler = (e: KeyboardEvent) => {
      ...
      if (containerRef.current?.contains(document.activeElement))
        (document.activeElement as any)?.blur();
      // Pause
      if ((keyboardShortcut === true || keyboardShortcut.pause) && (e.key === ' ' || e.key === 'k'))
  ...
  const videoProps: HTMLProps<HTMLVideoElement> & { src: string } = {
    ...
    onLoadedData: () => {
      ...
      let currentTime;
      if (playerKey) {
        currentTime = Number(localStorage.getItem(`${playerKey}-time`) as string);
      } else currentTime = 0;
    ...
    onTimeUpdate: () => {
      if (playerKey)
        localStorage.setItem(`${playerKey}-time`, String(playerRef.current?.currentTime || 0));
    ...
    children: (
      <>
        {!!subtitles?.length && subtitleIndex >= 0 && loadedData && (
          <track
            kind='subtitles'
  ...
  return (
    <>
      {poster && !pauseDidUpdate && <img src={poster} className='tuby-poster' alt='Tuby Poster' />}
      <div
        ref={containerRef}
        className={`tuby-container ${hoverEnabled ? 'tuby-controls-hovered' : ''}`}
      >
        {children ? children(playerRef, videoProps) : <video ref={playerRef} {...videoProps} />}
        ...
        >
          ...
          <div className='tuby-controls-main'>
            <div className='tuby-controls-left'>
              ...
              >
                {paused ? <Play className='tuby-icon-sm' /> : <Pause className='tuby-icon-sm' />}
              </button>
            ...
            <div className='tuby-controls-right'>
              {Boolean(subtitles) && (
                <button
                  ...
                  data-tuby-tooltips={internationalization?.tooltipsSubtitles || `Subtitles (c)`}
                  onClickCapture={() =>
                  ...
                  <div ref={ref} style={{ position: 'relative' }}>
                    <button
                      ...
                      {...(!settingsActive
                        ? {
                            'data-tuby-tooltips':
                              internationalization?.tooltipsSettings || 'Settings',
                          }
                        : {})}
                    >
              ...
              <button
                ...
                data-tuby-tooltips={`${
                  onFullScreen
                    ? internationalization?.tooltipsExitFullscreen || 'Exit full screen (f)'
                    : internationalization?.tooltipsFullscreen || 'Full screen (f)'
                }`}
